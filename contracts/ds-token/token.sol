...
contract Storage is Initialized{
    ...
    modifier onlyRebaser() {
        require(msg.sender == rebaser, "Token: only rebaser");
        _;
    }
    ...
    modifier onlyMinter() {
        require(msg.sender == minter, "Token: only minter");
        _;
    }
}
...
interface IToken is IERC20{
    ...
    //query
    function tokenToFragment(uint256 value) external view returns (uint256);
...
contract Token is Storage, IERC20{
    ...
    function transfer(address to, uint256 value) override external returns (bool) {
        _transferFragment(msg.sender, to, _tokenToFragment(value));
        emit Transfer(msg.sender, to, value);
        return true;
    }
    
    function transferFrom(address from, address to, uint256 value) override external returns (bool){
        uint256 fragmentValue = _tokenToFragment(value);
    ...
    function balanceOf(address who) override external view returns (uint256){
        return _fragmentToToken(fragmentBalances[who]);
    }

    function allowance(address owner_, address spender) override external view returns (uint256){
        return _fragmentToToken(allowedFragments[owner_][spender]);
    }
    ...
    function _mint(address to, uint256 amount) internal {
        ...
        require(scalingFactor <= _maxScalingFactor(), "Token: max scaling factor too low");
    ...
    function _tokenToFragment(uint value) internal view returns (uint256){
        return value.mul(Decimals).div(scalingFactor);
    }
}

contract HGBP is Token, IToken{
    using SafeMath for uint256;
    ...
    function rebaseByMilli(uint256 epoch, uint256 milli, bool positive) override external onlyRebaser returns (uint256){
        require(milli < 1000, "Token: milli need less than 1000");
    ...
    function fragmentToToken(uint256 value) override external view returns (uint256){
        return _fragmentToToken(value);
    }
    
    function tokenToFragment(uint256 value) override external view returns (uint256){
      return _tokenToFragment(value);
    }
    ...
    function _rebase(uint256 epoch, uint256 indexDelta, bool positive) internal returns (uint256){
        ...
        _totalSupply = _fragmentToToken(fragment);
        return _totalSupply;
    }
    ...
    function _initialize(string memory _name, string memory _symbol, uint8 _decimals, uint256 _initTotalSupply) internal noInit {
        ...
        fragment = _tokenToFragment(_initTotalSupply);
